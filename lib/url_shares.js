...
URLShares.prototype.count = function (url, callback) {
    debug('Getting share counts for ' + url);
    var remaining = 4
      , complete = false
      , counts = {
            facebook_likes: null
    ...
    function next(err) {
        if (complete) {
            if (err) {
                debug('Skipping error %s for url ' + url, err);
            }
            return;
        }
        if (err) {
            debug('Error %s for url ' + url, err);
            debug('Partial counts were ' + JSON.stringify(counts));
            complete = true;
            return callback(err, counts);
        }
        if (!--remaining) {
            debug(JSON.stringify(counts) + ' shares for ' + url);
            complete = true;
