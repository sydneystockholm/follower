...
URLShares.prototype.count = function (url, callback) {
    ...
    this.facebook.urlLikes(url, function (err, likes) {
        if (err) {
            return next(new Error('Error on facebook.urlLikes(' + url + '): ' + err.toString()));
        }
    ...
    this.twitter.urlTweets(url, function (err, tweets) {
        if (err) {
            return next(new Error('Error on twitter.urlTweets(' + url + '): ' + err.toString()));
        }
    ...
    this.pinterest.urlPins(url, function (err, pins) {
        if (err) {
            return next(new Error('Error on pinterest.urlPins(' + url + '): ' + err.toString()));
        }
    ...
    this.google.urlPlusOnes(url, function (err, plus_ones) {
        if (err) {
            return next(new Error('Error on google.plusOnes(' + url + '): ' + err.toString()));
        }
