 ...
 *
 * @param {String} url
 * @param {Mixed} id (optional) - emitted along with the url when share counts are obtained
 * @param {Function} callback
 */

URLShareQueue.prototype.count = function (url, id, callback) {
    ...
    this.getCachedShares(url, function (err, shares, timestamp) {
        ...
        if (!shares) {
            ...
            debug('%j shares for %s', shares, url);
...
URLShareQueue.prototype.add = function (url, id) {
    this.queue.push({ url: url, id: id });
        ...
        if (timestamp && timestamp + self.ttl > Date.now()) {
            ...
            self.url.count(url, function (err, shares) {
                if (err) {
                    ...
                    self.emit('error', err, url, next.id);
                    return self.queueNextTick();
                }
                self.emit('shares', url, shares, next.id);
                self.saveShareCounts(url, shares, function (err) {
                    if (err) self.emit('error', err);
                    debug('Saving %j for %s', shares, url);
                    self.queueNextTick();
                });
            });
        }
