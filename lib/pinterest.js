...
Pinterest.prototype.urlPins = function (url, callback) {
    ...
    this.request.json(url_, function (err, pins) {
        ...
        if (!('count' in pins)) {
            debug('Failed to get number of pins for %s', url);
            return callback(new Error('Invalid response'));
        }
        pins.count = Number(pins.count);
        debug('%s pins for %s', pins.count, url);
...
Pinterest.prototype.latestMedia = function (username, board, callback) {
    ...
    this.request.json(url + '/pins/', function (err, body) {
        ...
        try {
            ...
            if (board) {
                debug('Got %s items for user %s and board %s', media.length, username, board);
            } else {
            ...
            if (board) {
                debug('Failed to fetch media for user %s and board %s', username, board);
            } else {
                debug('Failed to fetch media for user %s', username);
            }
            return callback(new Error('Invalid response'));
        }
    });
