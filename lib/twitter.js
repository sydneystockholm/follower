...
Twitter.prototype.followerCount = function (username, callback) {
    ...
    this.request.json(url, function (err, user) {
        ...
        debug(user.followers_count + ' followers for user ' + username);
...
Twitter.prototype.urlTweets = function (url, callback) {
    ...
    this.request.json(url_, function (err, url) {
        if (err) return callback(err);
        if (!('count' in url)) {
            debug('Failed to get number of tweets for ' + url);
            return callback(new Error('Invalid response'));
        }
        debug(url.count + ' tweets for ' + url);
...
Twitter.prototype.bearerToken = function (callback) {
    ...
    if (this.key in bearer_tokens) {
        debug('Using cached bearer token for key ' + this.key);
        ...
        self.request.json({
          ...
          , headers: { 'Authorization': 'Bearer ' + token }
            ...
            try {
                ...
                debug('Got ' + tweets.length + ' tweets for user ' + username);
                callback(null, tweets);
            } catch (e) {
                debug('Failed to get tweets for user ' + username);
                callback(new Error('Invalid response'));
            }
        });
    });
        ...
        self.request.json({
          ...
          , headers: { 'Authorization': 'Bearer ' + token }
            ...
            try {
                ...
                debug('Got ' + tweets.length + ' tweets for hashtag ' + hashtag);
                callback(null, tweets);
            } catch (e) {
                debug('Failed to get tweets for hashtag ' + hashtag);
                callback(new Error('Invalid response'));
            }
        });
    });
